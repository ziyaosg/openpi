...
OUTPUT_DIR = "/home/ziyao/Documents/policy_records_20260304_183031/gradcam_grids_folders"
...
def episode_for_step(step: int, episodes: List[EpisodeInfo]) -> EpisodeInfo:
    ...
    NOTE: This assumes end_idx is INCLUSIVE (matches your example).
    If your end_idx is actually EXCLUSIVE, change condition to:
        ep.start_idx <= step < ep.end_idx
    """
    for ep in episodes:
...
def main() -> None:
    ...
    for f in files:
        ...
        ep_folder = f"ep{ep.episode_num}_{str(ep.success).lower()}"  # "true"/"false"
