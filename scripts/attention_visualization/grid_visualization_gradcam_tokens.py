#!/usr/bin/env python3
"""GradCAM attribution per decoded action token.
...
Layout (6 rows × 4 modality columns):
  ...
  Row 1-5: GradCAM for action token 0, 1, 2, 3, 4
"""
...
from .grid_visualization_raw_alpha import CLIP_PERCENTILE, GAMMA
...
def process_one(npy_path: str, out_png: str) -> None:
    ...
    for tok_idx in range(NUM_ACTION_TOKENS):
        ...
        for cam in CAM_NAMES:
            h = to_2d_heatmap(rec[f"outputs/debug/gradcam/action_tokens/{tok_idx}/{cam}"], cam)
            heats.append(np.maximum(h, 0.0) if APPLY_RELU else h)
        image_heats_per_tok.append(normalize_joint(heats))
