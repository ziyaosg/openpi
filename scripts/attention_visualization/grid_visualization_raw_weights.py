...
def process_one(npy_path: str, out_png: str) -> None:
    ...
    for cam_name, img_key in zip(CAM_NAMES, CAM_IMAGE_KEYS):
        ...
        for k in (span_key, grid_key, img_key):
            if k not in rec:
    ...
    heats, imgs = [], []
    for (span, grid_hw), (cam_name, img_key) in zip(zip(cam_spans, cam_grids), zip(CAM_NAMES, CAM_IMAGE_KEYS)):
        # slice the token positions that belong to this camera from the full attn row,
        # then reshape from flat (Hp*Wp,) back to the patch grid (Hp, Wp)
